...
**Для кого:** системный администратор с правами `sudo` на сервере, где уже
запущены llama.cpp (порт 8000) и сервис эмбеддингов (порт 8001).

**Что получится:** веб-интерфейс на порту **8080**, работающий как systemd-сервис
(автозапуск, перезапуск при сбое), с обязательной аутентификацией пользователей.

Приложение — один процесс Python (FastAPI + uvicorn), БД — файл SQLite, GPU не
требуется. Порт меняется в `.env`.

Все команды предполагают развёртывание в `/opt/srv-ai-ui`. Выполняй по порядку
сверху вниз.

> Для проверки на своём компьютере до серверного развёртывания — см.
> `docs/local-test-quick.md` (быстро) и `docs/local-test-full.md` (со всем).
...
лежат в `static/` — из сети ничего не тянется. Фильтр ПДн определяет имена
правилами (в `app/pii.py`) — внешних NER-моделей и их загрузки не требует.
