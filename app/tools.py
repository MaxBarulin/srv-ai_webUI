...
CALC_TOOLS: list[dict] = [
    _tool("calc_run",
          ...
          "ссылаться на имена предыдущих. Второй вызов в одном ответе не "
          "нужен — имена из прошлого вызова в нём недоступны, и число "
          "пришлось бы переписывать вручную.",
          {
              "steps": {
                  "type": "array",
...
CALC_SYSTEM_HINT = (
    ...
    "В ответе показывай ход расчёта: формулу, значение и единицу по каждому шагу.\n"
    "Весь расчёт — один вызов. Промежуточная величина это отдельный ШАГ внутри "
    "того же вызова, а не второй вызов. Понадобилось досчитать — добавь шаги в "
    "ту же цепочку. Никогда не переписывай в новый вызов число, полученное из "
    "предыдущего: перенос числа руками — та самая ошибка, ради которой "
    "инструмент и существует.\n"
    "Если для расчёта не хватает исходных данных — не подставляй значения сам, "
    "не считай приблизительно и не подбирай «типовые». Задай пользователю "
    "короткий вопрос, каких величин не хватает, и дождись ответа."
)
