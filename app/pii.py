...
# Отчества (с учётом падежей). Мужские -ович/-евич уникальны — маскируем даже
# в одиночку. Женские -овна/-евна маскируем ТОЛЬКО в связке с предшествующим
# именем (см. _name_spans): одиночные заглавные слова на -овна редки, но бывают
# (Царевна). Окончание -ична НЕ берём суффиксом — оно совпадает с массой
# прилагательных (Симметрична, Аналогична, Идентична, Практична); редкие
# отчества этого типа (Никитична и т.п.) заданы закрытым списком.
_PATRO_MALE = re.compile(r"^[А-ЯЁ][а-яё]+(?:ович|евич)(?:а|у|ем|е)?$")
_PATRO_FEM = re.compile(
    r"^[А-ЯЁ][а-яё]+(?:овн|евн)(?:а|ы|е|у|ой)$"
    r"|^(?:Ильинич|Кузьминич|Никитич|Фоминич|Саввич|Лукинич)(?:на|ны|не|ну|ной)$")

# Инициалы: «Фамилия И.О.» или «И.О. Фамилия»
_INITIALS = re.compile(rf"\b{_CAP}\s+[А-ЯЁ]\.[\s]?[А-ЯЁ]\.|"
                       rf"\b[А-ЯЁ]\.[\s]?[А-ЯЁ]\.\s+{_CAP}")

# Слова-заглушки бланков (например, «И.О. Фамилия» в строке подписи) — это не ПДн
_TEMPLATE_WORD = re.compile(r"\b(?:Фамилия|Имя|Отчество)\b")
...
def _name_spans(text: str) -> list[tuple[int, int]]:
    ...
    # 1) Инициалы «Фамилия И.О.» / «И.О. Фамилия» (кроме шаблона бланка)
    for m in _INITIALS.finditer(text):
        if _TEMPLATE_WORD.search(m.group(0)):
            continue
    ...
    for i, (w, s, e) in enumerate(toks):
        wl = w.lower()
        if _PATRO_MALE.match(w) or _PATRO_FEM.match(w):
            ...
            while (k - 1 >= 0 and i - (k - 1) <= 2
                ...
                k -= 1
            # мужское отчество уникально — маскируем даже одиночное; женское
            # -овна/-евна маскируем только в связке с именем (k < i), иначе это
            # чаще прилагательное/существительное с заглавной (Царевна, Виновна)
            if _PATRO_MALE.match(w) or k < i:
